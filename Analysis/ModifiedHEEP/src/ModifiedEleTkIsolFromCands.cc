			    ...
			    const PIDVeto pidVeto)const
{
  ...
  for(auto& cand  : cands) {
    ...
    //  if(fabs(addTrk.eta()-trk.eta())<cuts.minDEta && fabs(addTrk.phi()-trk.phi())<0.05) continue;
...
{
  return calIsol(eleTrk.eta(),eleTrk.phi(),eleTrk.vz(),tracks,addTrk,nrMatchedTrk,rtMatchedTrk,eleTrk.pt());
}
...
ModifiedEleTkIsolFromCands::calIsol(const double eleEta,const double elePhi,
          ...
          const reco::TrackBase& addTrk,
          int& nrMatchedTrk, float& rtMatchedTrk, double elePt)const
{
  double ptSum=0.;
  int nrTrks=0;
  ...
  for(auto& trk : tracks){
    if(passTrkSel(trk,trk.pt(),cuts,eleEta,elePhi,eleVZ)){
      // if(reco::deltaR2(addTrk.eta(),addTrk.phi(),trk.eta(),trk.phi()) < cuts.addGsfVetoDR2) {
      if(std::abs(addTrk.eta()-trk.eta()) < cuts.dEta2nd && std::abs(addTrk.phi()-trk.phi()) < cuts.dPhi2nd) {
        nrMatchedTrk++;
        rtMatchedTrk = addTrk.pt()/trk.pt();
        continue;
				    ...
				    const double eleVZ)
{
  ...
  //  std::abs(dEta)>=cuts.minDEta &&
    ...
    trk.hitPattern().numberOfValidPixelHits() >= 1 &&
    //(trk.ptError()/trkPt < cuts.maxDPtPt || cuts.maxDPtPt<0) &&
    // passQual(trk,cuts.allowedQualities) &&
    //passAlgo(trk,cuts.algosToReject) &&
